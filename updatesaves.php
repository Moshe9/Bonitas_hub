...
// $sqlDelete = "DELETE FROM tbl_advisors WHERE user_id = ".intval($_REQUEST['userid']);
...
if($currentTab === '0'){
    ...
    // $updated_at = CURDATE();
    if($userId === 'empty'){
        $sql = "INSERT INTO tbl_advisors (medstart_date,capturedby,brokercode,brokeragename,brokername,brokeremail,brokertel, created_at, updated_at)
    ...
    $sql = "UPDATE tbl_advisors SET medstart_date = '$medicalstart',capturedby = '$capturedby',brokercode = '$brokercode',brokeragename = '$brokeragename',brokername = '$brokername',brokeremail = '$brokeremail',brokertel = '$brokertel',
...
if($currentTab === '1')
{
    ...
    // $get_id = mysqli_query($conn,"SELECT user_id FROM tbl_members WHERE user_id = $userId");
    ...
    $sql = "UPDATE tbl_advisors SET policy_option = '$policyoption' WHERE user_id = '$userId'";
        if (mysqli_query($conn, $sql)) {
...
if($currentTab === '2'){
    ...
    if($result != NULL)
    {
        ...
        // var_dump($result);
    if($result != $userId){
        $sql = "INSERT INTO tbl_members (user_id,
        idPassType,
        mainid_number,
        main_title,
        main_surname,
        ...
        $sql = "UPDATE tbl_members SET idPassType = '$idPassType',mainid_number = '$mainIdnumber',main_title = '$maintitle',main_surname = '$mainsurname',main_firstname = '$mainfirstname',main_initial = '$maininitial',main_marital = '$mainmarital',
...
if($currentTab == 3){
    ...
    $get_id = mysqli_query($conn,"SELECT user_id FROM tbl_members_contacts WHERE user_id = $userId");
    $result = mysqli_fetch_row($get_id);
    if($result != NULL)
    {
        $result =  $result[0];
    ...
    // echo 'This is the result : ' . $result . " = " . $userId  ."<br>";
        //  var_dump($result);
    if($result != $userId){
        $sql = "INSERT INTO tbl_members_contacts (user_id,
        m_cell,
        m_telh,
        m_telw,
        mtax,
        ...
        '$email','$pobox','$postalstreetnum','$postalstreetname','$postalstreetntype','$postalsuburb','$postalcode','$postalstreetnum',
        '$postalstreetname', '$postalstreetntype', '$postalsuburb', '$postalcode')";
        if (mysqli_query($conn, $sql)) {
            echo "New record has been added successfully ! ";
        } else {
            echo "Error: " . $sql . ":-" . mysqli_error($conn);
        ...
        $sql = "UPDATE tbl_members_contacts as mc, tbl_advisors as a SET mc.m_cell = '$cell',mc.m_telh = '$hometel',mc.m_telw = '$worktel',mc.mtax = '$taxnumber',mc.comm_method= '$comm_method',
            ...
            mc.m_psuburb = '$postalsuburb',mc.m_postalcode = '$postalcode',mc.m_ssnum = '$postalstreetnum',mc.m_ssname = '$postalstreetname'
          ,mc.m_sstype = '$postalstreetntype',mc.m_ssuburb = '$postalsuburb',mc.m_spostalcode = '$postalcode', a.updated_at = now() WHERE mc.user_id = '$userId' AND a.user_id = '$userId'";
        if (mysqli_query($conn, $sql)) {
...
if($depsconst == 'yes' && $currentTab == 4){
    ...
    $get_id = mysqli_query($conn,"SELECT COUNT(*) FROM tbl_dependents WHERE user_id = $userId");
    $result = $get_id->fetch_row();

    if($result[0] > 0)
    {

        $delete = ("DELETE FROM tbl_dependents WHERE user_id = $userId");
    ...
    while($counter2 < $counter)
    {
        ...
        $taxnum = $depArray[$counter2][10];
            ...
            $sql = "INSERT INTO tbl_dependents (user_id,
            dep_title,
            d_idpass,
            d_firstname,
            d_surname,
...
if(($depsconst == 'no' && $currentTab == 4) || ($depsconst == 'yes' && $currentTab == 5))
{
    ...
    $get_id = mysqli_query($conn,"SELECT user_id FROM tbl_banks_contributions WHERE user_id = $userId");
    $result = mysqli_fetch_row($get_id);
    if($result != NULL)
    {
        $result =  $result[0];
    ...
    if($result != $userId){
        $sql = "INSERT INTO tbl_banks_contributions (user_id,
        thirdpartycheck,
        bankname,
        accholderid,
        accholder,
        ...
        $sql = "UPDATE tbl_banks_contributions SET thirdpartycheck = '$thirdpartycheckfield',bankname = '$contribbank',accholderid = '$contribaccholderid',accholder = '$contribaccholdname',
    ...
    $id = mysqli_query($conn,"SELECT user_id FROM tbl_banks_refunds WHERE user_id = $userId");
    ...
    if($resultId != $userId){
        $sql = "INSERT INTO tbl_banks_refunds (user_id,
        thirdpartycheck,
        sameacccheck,
        bankname,
        accholderid,
        ...
        $sql = "UPDATE tbl_banks_refunds SET thirdpartycheck = '$refthirdpartycheckfield',sameacccheck = '$checkcontribcopy', bankname = '$refundbank',accholderid = '$refundaccholderid',
...
if(($depsconst == 'no' && $currentTab == 5) || ($depsconst == 'yes' && $currentTab == 6)){
   ...
   $userId = intval($userId);
    ...
    $get_id = mysqli_query($conn,"SELECT COUNT(*) FROM tbl_medical_scheme WHERE user_id = $userId");
    $result = $get_id->fetch_row();

    if($result[0] > 0)
    {

        $delete = ("DELETE FROM tbl_medical_scheme WHERE user_id = $userId");
        if (mysqli_query($conn, $delete)) {
    ...
    while($counter2 < $counter )
    {
        ...
        $comupload = $pmArray[$counter2][5];
            ...
            $sql = "INSERT INTO tbl_medical_scheme(
                    user_id,
                    fullname,
                    medscheme,
                    membership_num,
...
if(($depsconst == 'no' && $currentTab == 6) || ($depsconst == 'yes' && $currentTab == 7))
{
    ...
    $get_id = mysqli_query($conn,"SELECT COUNT(*) FROM tbl_medical_questions WHERE user_id = $userId");
    $result = $get_id->fetch_row();

    if($result[0] > 0)
    {

        $delete = ("DELETE FROM tbl_medical_questions WHERE user_id = $userId");
        if (mysqli_query($conn, $delete)) {
    ...
    foreach($amqArray as $mqArray)
    {
        foreach($mqArray as $value)
        {
            ...
            $sql = "INSERT INTO tbl_medical_questions(
                  user_id,
                  medical_questions,
                  fullname,
                  illness,
                  ...
                  gpfullname
                     ...
                     VALUES ('$userId','$medicalquestion','$fullname','$illness','$treated', '$firsttreatmentdate','$lasttreatmentdate', '$gpfullname')";
                    if(mysqli_query($conn, $sql)){
                        echo "New record has been added successfully !";
                    }else{
                        echo "Error: " .$sql . ":-" . mysqli_error($conn);
...
if(($depsconst == 'no' && $currentTab == 7 && $optionsconst == 'select' ) || ($depsconst == 'yes' && $currentTab == 8 && $optionsconst == 'select')){
   ...
   $userId = intval($userId);
    ...
    $get_id = mysqli_query($conn,"SELECT COUNT(*) FROM tbl_gpnominations WHERE user_id = $userId");
    $result = $get_id->fetch_row();

    if($result[0] > 0)
    {

        $delete = ("DELETE FROM tbl_gpnominations WHERE user_id = $userId");
        if (mysqli_query($conn, $delete)) {
    ...
    while($counter2 < $counter )
    {
        ...
        //insert
        $sql = "INSERT INTO tbl_gpnominations(
            user_id,
            fullname,
            firstdocname,
            firstdocpnum,
...
if(($depsconst == 'no' && $currentTab == 8 && $optionsconst == 'select' ) || ($depsconst == 'yes' && $currentTab == 9 && $optionsconst == 'select') || ($depsconst == 'yes' && $currentTab == 8 && $optionsconst != 'select') || ($depsconst == 'no' && $currentTab == 7 && $optionsconst != 'select')){
    ...
    $sql = "UPDATE tbl_advisors SET terms_agreement = '$terms_agreement',

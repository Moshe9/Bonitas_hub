function db_saves() {
    ...
    if(currentTab == 1)
    {
        ...
        boncapamount = document.getElementById('boncap-amnt');
        ...
        if (null != boncapamount) {
            boncapamount = document.getElementById('boncap-amnt').value;
        } else {
            boncapamount = '';
        }
        $.ajax({
            method: "POST",
            url: "/includes/saves.php",
            data: {currenttab: currenttab, userId: userId, policyoption: policyoption, incomebound: incomebound, boncapamount: boncapamount }
            })
            .done(function( response ) {
                toastr.options = {
                    "closeButton": true,
    ...
    if(currentTab == 3)
    {
        ...
        $.ajax({
            ...
            data: {currenttab: currenttab, userId: userId,
                ...
                postalstreetname: postalstreetname, postalstreetntype: postalstreetntype, postalsuburb: postalsuburb, postalcode: postalcode, postalcode: postalcode, comm_method: comm_method,
    ...
    if ((depsconst == 'no' && currentTab == 4) || (depsconst == 'yes' && currentTab == 5)) {
        ...
        if (document.getElementById('refthirdpartycheckfield').value != 'yes') {
            refundaccholderid = document.getElementById("mianid-number").value;
            refundaccholder = document.getElementById('main-firstname').value;
            refundaccholdsurname = document.getElementById('main-surname').value;
        ...
        banking = {
                'userId': userId,
                'contribbank': contribbank,
                'contribaccholdname': contribaccholdname,
                'contribaccnum': contribaccnum,
                'contribacctype': contribacctype,
                'contribaccholderid': contribaccholderid,
                'contribaccholdsurname': contribaccholdsurname,
                'contrdebidate': contrdebidate,
                'refundbank': refundbank,
                'refundaccholder': refundaccholder,
                'refundaccnum': refundaccnum,
                'refundacctype': refundacctype,
                'refundaccholderid': refundaccholderid,
                'refundaccholdsurname': refundaccholdsurname,
                'checkcontribcopy': checkcontribcopy,
                'refthirdpartycheckfield': refthirdpartycheckfield,
                'thirdpartycheckfield': thirdpartycheckfield,
    ...
    //gp noms
    if ((depsconst == 'no' && currentTab == 7) || (depsconst == 'yes' && currentTab == 8)) {
        currenttab = currentTab;
        userId = document.getElementById('userid').value;
    ...
    if ((depsconst == 'no' && currentTab == 8) || (depsconst == 'yes' && currentTab == 9) || (depsconst == 'yes' && currentTab == 8) || (depsconst == 'no' && currentTab == 7)) {
        currenttab = currentTab;
        userId = document.getElementById('userid').value;

    ...
    <body>
        ...
        <header class="page-topbar" id="header">
            <div class="navbar navbar-fixed ">
                <nav style="background: #01468c;">
                    <div class="nav-wrapper">
                        <a href="https://forms.afrocentricds.com/hub-home" class="brand-logo"></a>
                        <ul class="right hide-on-med-and-down">
                            <li><a href="https://forms.afrocentricds.com/hub-home">HOME</a></li>
                            <li><a href="https://forms.afrocentricds.com/marketing-elements">MARKETING ELEMENTS</a></li>
                            <li><a href="https://newaccount1613558039243.freshdesk.com/support/home" target="_blank">SYSTEM SUPPORT</a></li>
                            <li><a href="https://forms.afrocentricds.com/my-account/11">MY ACCOUNT</a></li>
                            <li><a href="https://forms.afrocentricds.com/logout"><i class="fa fa-sign-out"></i></a></li>
                        </ul>
                    </div>
                </nav>
            </div>
...
if ($conn->connect_error) {
  die("Connection failed: " . $conn->connect_error);
    ...
    $policyoptions = ['PL477'=>"BonComprehensive", 'PL740'=>"Boncap", 'PL476'=>"BonClassic", 'PL472'=>"BonComplete", 'PL240'=>"BonSave", 'PL475'=>"BonFit Select", 'PL020'=>"Standard", 'PL021'=>"Standard Select", 'PL277'=>"Primary", 'PL469'=>"Primary Select", 'PL464'=>"BonStart", 'PL465'=>"BonStart plus", 'PL474'=>"Hospital Standard", 'PL479'=>"BonEssential", 'PL471'=>"BonEssential Select"];
    ...
    if ($_POST['contrib-bankname'] === 'ABSA') {
        $contributionbankname = 'Absa';
        $contribBranchCode = 632005;
    }
    if ($_POST['contrib-bankname'] === 'AFRICAN') {
        $contributionbankname = 'African';
        $contribBranchCode = 430000;
    }
    if ($_POST['contrib-bankname'] === 'MEEG') {
        $contributionbankname = 'Capitec';
        $contribBranchCode = 470010;
    }
    if ($_POST['contrib-bankname'] === 'DISC_BANK') {
        $contributionbankname = 'Discovery';
        $contribBranchCode = 679000;
    }
    if ($_POST['contrib-bankname'] === 'FNB') {
        $contributionbankname = 'FNB';
        $contribBranchCode = 250655;
    }
    if ($_POST['contrib-bankname'] === 'INVESTEC') {
        $contributionbankname = 'Investec';
        $contribBranchCode = 580105;
    }
    if ($_POST['contrib-bankname'] === 'NEDCOR') {
        $contributionbankname = 'Nedbank';
        $contribBranchCode = 198765;
    }
    if ($_POST['contrib-bankname'] === 'STANDARD') {
        $contributionbankname = 'Standard';
        $contribBranchCode = 51001;
    }
    if ($_POST['contrib-bankname'] === 'TYMDG_BANK') {
        $contributionbankname = 'Tyme';
        $contribBranchCode = 678910;
    }

    //Build the Dependents
    echo "User ID is : " . $userId;
    $deps = mysqli_query($conn, "SELECT * FROM fed_tbl_dependents WHERE user_id = $userId");
    if(!empty($deps))
    {
    ...
    $prevmeds = mysqli_query($conn, "SELECT * FROM fed_tbl_medical_scheme WHERE user_id = $userId");
    if(!empty($prevmeds))
    {
        if(mysqli_num_rows($prevmeds) > 0)
        {
            ...
            {
                // echo "ID: " .$row['fed_tbl_id'] . " User ID: " .$row['user_id'] . "Full Name: " .$row['fullname'] . " Medical Scheme: " .$row['medscheme'] . " Membership Number: " .$row['membership_num'] . " Join Date: " .$row['medjoindate'] . " End Date: " .$row['medenddate'] . " Upload: " .$row['comupload'] . '<br/><br/>';
                // array start
    ...
    //GP Noms Build
    $gpnoms = mysqli_query($conn, "SELECT * FROM fed_tbl_gpnominations WHERE user_id = $userId");
    if(!empty($gpnoms))
    {
    ...
    $medicals = mysqli_query($conn,"SELECT * FROM fed_tbl_medical_questions WHERE user_id = $userId");
    if(!empty($medicals))
    {
    ...
    // {
    //     //db
    //     $depCount = mysqli_query($conn, "SELECT COUNT(*) FROM fed_tbl_dependents WHERE user_id = '$userId'");
    //     echo $depCount;
    // }
    if(!empty($_POST["depcounter"]))
    {
                    ...
                    "address": [
                        {
                            ...
                            "streetName": "'.substr($_POST['m-ssname'], 0,6).'",
    ...
    if(!empty($prevmeddetails)){
        ...
        $subdata .= '"medicalAid": [';
            while($prevmedCount <= $pmCount){
                $subdata .= '
                    {   
                        "medicalAidName": "'.$prevmeddetails[$prevmedCount]['med_scheme'].'",
                        "memberNumber": "'.$prevmeddetails[$prevmedCount]['member_number'].'",
                        "startDate": "'.$prevmeddetails[$prevmedCount]['join_date'].'",
                        "endDate": "'.$prevmeddetails[$prevmedCount]['end_date'].'"
                    }';
                

                if($prevmedCount == $pmCount)
                {
                    break;
                }
                $subdata .=',';
    ...
    if(!empty($medqdetails))
    {
        $subdata .= '"conditions":{'; 
            ...
            while($mqCount <= $medqCount)
            {
                $subdata .= '
                    {
                        "questionCode": "'.$medqdetails[$mqCount]['mqcode'].'",
                        "conditionCode": null,
                        "detail": "'.$medqdetails[$mqCount]['illness'].'",
                        "current": "'.$medqdetails[$mqCount]['treated'].'",
                        "lastTreatmentDate": "'.$medqdetails[$mqCount]['last_treatDate'].'",
                        "receivedTreatment": "'.$medqdetails[$mqCount]['treated'].'",
                        "treatingDoctor": "'.$medqdetails[$mqCount]['gp_fullname'].'",
                        "conditionResolved": null
                    }';
                

                if($mqCount == $medqCount)
                {
                    //$subdata .=',';
                    break;
                }
...
$get_status = mysqli_query($conn,"SELECT status FROM fed_tbl_advisors WHERE user_id = $userId");
...
if($result[0] != '1')
{
	...
	{
	    $status = -1;
	    $sql = "UPDATE fed_tbl_advisors SET status = '$status', errormessage = '$errormessage',errorcode = '$errorcode' WHERE user_id = '$userId'";
	...
	{
        $membershipnum = ltrim($membershipmessage, "Online Application Loaded successfully for MemberShip Number : ");
	    $status = 1;
	    $sql = "UPDATE fed_tbl_advisors SET status = '$status', reference_id
	     = '$submitqueryid', membership_num = '$membershipnum' WHERE user_id = '$userId'";
	    if(mysqli_query($conn, $sql)){
		echo "Record has been updated successfully !";
	    }else{
		echo "Error: " . $sql . ":-" . mysqli_error($conn);
	    ...
	    echo "<p style = 'color: #ba0c35; font-weight: 400;'>There's an error reaching nexus, please try again later</p>";
        $status = -1;
        $sql = "UPDATE fed_tbl_advisors SET status = '$status', errormessage = 'Error reaching Nexus Submission', errorcode = 'F1001' WHERE user_id = '$userId'";
	    if(mysqli_query($conn, $sql)){
...
$message = '<div style="width:100%;background: #d4d4d4;font-size:25px;margin:0px;padding:0px;overflow:wrap;"><table style="width:100%; font-size: 25px;margin:0px;padding:0px;overflow: wrap;" CELLSPACING=0>
...
$message .=  '<tr style="background: #3b3838;color: #ffffff;"><td colspan="2" style="height: 40px;margin: 0;padding: 0 10px;"><h3 style="margin: 0;color: #ffffff;">Submission Details</h3></td></tr>
  ...
  <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">Error Message</td><td style="padding: 0 10px;color: #9d0000;">'. wordwrap($errormessage,50,"<br>\n",TRUE).'</td></tr>';
$message .=  '<tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;"></td><td style="padding: 0 10px;"></td></tr>
  <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;"></td><td style="padding: 0 10px;"></td></tr>';
}
...
$message .=  '<tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">Broker Email</td><td style="padding: 0 10px;">'. $_POST['advisoremail'] .'</td></tr>
  ...
  <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">ID Validation Notice</td><td style="page-break-after:always;"><p>'. wordwrap($idvalidnotice,50,"<br>\n",TRUE) .'</p></td></tr>
...
if(!empty($depdetails)){
    ...
    while($dependantsCount <= $depCount){
        ...
        <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">ID Validation Notice</td><td style="overflow:wrap; width: 72%;padding: 0 10px;overflow-wrap: break-all; whitespace:normal;"><p>'. $_POST['d'.$dependantsCount.'-idvalidnotice'] .'</p></td></tr>
...
$message .=  '<tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;"></td><td style="padding: 0 10px;"></td></tr>
  ...
  <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">Branch Code</td><td style="padding: 0 10px;">'. $contribBranchCode .'</td></tr>';
  if($_POST['thirdpartycheckfield'] === 'yes'){
    ...
    <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">Account Holder ID Validation</td><td style="overflow:wrap; width: 72%;padding: 0 10px;overflow-wrap: break-all; whitespace:normal;">'. $_POST['thirdpartyidvalid'] .'</td></tr>';
  }
  else{
  ...
  <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">Account Validation Message</td><td style="overflow:wrap; width: 72%;padding: 0 10px;overflow-wrap: break-all; whitespace:normal;"><p>'.wordwrap($_POST['contribverification'],50,"<br>\n",TRUE).'</p></td></tr>
...
if(!empty($medqdetails)){
    ...
    while($mqCount < $medqCount){
        ...
        <tr><td style="width: 22%;border-right: solid 2px #3b3838;padding: 5px 10px;">Medication</td><td style="padding: 0 10px;">'. $medqdetails[$mqCount]['medication'] .'</td></tr>';
        $mqCount++;
    }
...
$dompdf->set_paper(array(0,0,975,1316), "landscape" );
$dompdf->render();
...
$files = mysqli_query($conn,"SELECT * FROM fed_tbl_banks_contributions WHERE 'user_id' = $userId");
...
// com 1 upload
$files = "SELECT comupload FROM fed_tbl_medical_scheme WHERE 'user_id' = $userId";
...
//DOMPDF Ends=====================================================
//Additional Uploads
$prevmedcheck = mysqli_query($conn,"SELECT * FROM fed_tbl_medical_scheme WHERE 'user_id' = $userId");

$totalprevs = mysqli_num_rows($prevmedcheck);
if($submitsatus == 'OK')
{
 ?>
    <div class="full text flexrow">
        <div class="full text flexrow header">
        ...
        <div class="half text flexrow" id="uploads">
            <div class="full text flexrow">
                ...
                <p style="color: orange; display: none;" id="docunotice"><i class="fa fa-exclamation-triangle fa-1"></i> Please note the documents listed in Red are highly required.</p>
                <ul class="ul_checks">
                    <li><i class="fa fa-file-pdf"></i> Copy of ID</li>
                    <li><i class="fa fa-file-pdf"></i> Copy of Passport</li>
                    <li><i class="fa fa-file-pdf"></i> Copy of Marriage Certificate</li>
                    <li><i class="fa fa-file-pdf"></i> Proof of Registration (if Dependent is a student)</li>
                    <li id="thirdparty"><i class="fa fa-file-pdf"></i> Bank Statement for Third Party Account/ Company Account</li>
                    <li id="boncap"><i class="fa fa-file-pdf"></i> Boncap Option Document</li>
                    <li id="comtext"><i class="fa fa-file-pdf"></i> Certificate of Membership</li>
                </ul>
            </div>
        ...
        <div class="half text flexrow">
            <div class="input-field">
                ...
                <h4 id="additionalattachmsg" style="display: none; clear: both; color: orange;">Files uploaded successfully, please click the below button to attach to the submission.</h4>
                <p style="display: none; color: orange; font-weight: bold;" id="additionalattachlabel"><i class="fa fa-circle-check"></i> Great! The files you just uploaded needs to be attached to your application.</p>
                <input type="button" id="additionalattach" class="filesattach" value="Click to Attach files" onclick="additionalattach()" style="display: none;">
                <p style="color: green; font-weight: bold;">To finalise your application, please click the button below</p>
                <input type="button" id="summaryattach" class="summaryattach" value="Submit Application" onclick="summaryattach()">
            </div>
        </div>
        <script>
...
if($submitsatus != 'OK')
{
    ...
    function sendMembership($output){
        ...
        try {
            ...
            $mail->send();
            $messagestatus = "sent";
            echo 'Message has been sent, if you do not receive your email shortly please ensure to save this page as a PDF.';
        } catch (Exception $e) {
            echo "Message could not be sent. Mailer Error: {$mail->ErrorInfo}";
        }
    }
